...
- In R there is no such step: R code is handed to an **interpreter** instead
  (we come back to what that means).
...
::: {.callout-tip appearance="simple"}
The interpreter **never leaves**: the arithmetic is one CPU instruction, and
everything around it is a hundred more.
:::
