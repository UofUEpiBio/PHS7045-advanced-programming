...
- Through the **command line**:
  ...
  - The **GitHub CLI** [(link)](https://cli.github.com), `gh`, for everything that touches GitHub.

- Through a **standalone Git GUI**:
...
system -- your `.git` folder is a complete repository.
...
and [Codeberg](https://codeberg.org) do the same thing, and universities often self-host GitLab.
...
::: {.fragment .fade-in}
- **Remote**: a copy of your repository somewhere else. Each remote has has a short name, so you don't have to retype the URL. By convention the
...
::: {.fragment .fade-in}
To check what is going on (your first debugging step whenever a push is rejected):
...
::: {.fragment .fade-in}
Do the manual version at least once. Then never do it again.
:::
...
## Not a terminal person? GitHub Desktop
...
::: {.fragment .fade-in}
Caveats: **no official Linux version** (so it is not an option on CHPC), and it hides what Git is
actually doing.
:::
