...
::: {style="font-size: 20pt"}
  ...
  - Have a look at some R-package-dev practices based mostly on my own
  ...
  - We won’t talk about software engineering/design, rather we’ll take a
    look at some tools and dev cycle. (For that you have [“Code
...
2.  Already standarized: So you don’t need to think about how to
    structure it.
...
And files
  ...
  - **NAMESPACE** Declares the name of the objects (functions mostly)
...
1.  `use_readme_rmd()`: Creates a readme file that will be in the main
    folder of the project. Think about it as the home page of your
    project.

2.  `use_testthat()`: Will create the basic infrastructure for package
    testing.
...
::: {.column width="40%"}
  ...
  - You’ll need to update the `man/*Rd` every time that you add new
    roxygen content. Just press Ctrl + Shift + D and RStudio will do it
    for you.
...
The `Suggests` and `RoxygenNote` fields where added automagically by
    `devtools`.
...
## Step 3: Submit your package to CRAN/Bioconductor
  ...
  - The devtools package was built to make life easier. Such is it’s
    creator’s confidence on the `release` function that it comes with a
    warranty\!
